...
> 当前状态：MVP `v0.1.0` 已完成。CSV 分类、回归和纯数据清洗可以完成上传、字段确认、建议审阅、预处理与 CSV/HTML 结果导出；范围与后续计划见 [开发路线图](docs/roadmap.md)。
...
浏览器访问 `http://localhost:5173`。首页用于了解产品，点击“开始使用”后进入项目库；完整流程统一为“创建项目 → 数据概览 → 字段确认 → 质量诊断 → 处理方案 → 执行处理 → 结果导出”。项目库支持搜索和回收站，普通移除可以撤销或恢复，永久删除需要再次确认。

> 前端开发服务器当前监听 `localhost`。如果 `http://127.0.0.1:5173` 无法访问，请使用上面的 `localhost` 地址；后端仍使用 `http://127.0.0.1:8000`。
